...
def eval_rbbox_map(det_results,
    ...
    # NOTE: yan add mprecision and mrecall metrics
    mrecall, mprecision = print_map_summary(mean_ap, eval_results, dataset, area_ranges, logger=logger)
    return mean_ap, mprecision, mrecall, eval_results
...
def print_map_summary(mean_ap,
    ...
    # NOTE: yan, add mprecision mrecall metric
    header = ['class', 'gts', 'dets', 'recall', 'precision', 'ap']
    mrecall, mprecision = 0, 0
    for i in range(num_scales):
        if scale_ranges is not None:
        ...
        for j in range(num_classes):
            row_data = [
                ...
                # NOTE: yan, add mprecision mrecall metric
                f'{recalls[i, j]:.4f}', f'{precisions[i, j]:.4f}', f'{aps[i, j]:.4f}'
            ]
            mrecall += recalls[i, j]
            mprecision += precisions[i, j]
            table_data.append(row_data)
        # NOTE: yan, add mprecision mrecall metric
        mrecall /= num_classes
        mprecision /= num_classes
        table_data.append(['mAP', '', '', f'{mrecall:.4f}', f'{mprecision:.4f}', f'{mean_ap[i]:.4f}'])
        table = AsciiTable(table_data)
        table.inner_footing_row_border = True
        print_log('\n' + table.table, logger=logger)

    # NOTE: yan, add mprecision mrecall metric
    return mrecall, mprecision
