...
class RotatedSingleStageDTLoss(nn.Module):
    ...
    def forward(self, teacher_logits, student_logits, ratio=0.01, teacher_preds=None, img_metas=None):
        ...
        if self.loss_type in ['pr_origin_p5']:
            ...
            # 对每个 tensor 应用 sigmoid 函数
            teacher_probs = [torch.sigmoid(tensor) for tensor in t_cls_scores]#teacher_prob是一个元素为tensor的列表
            # 用于存储每个张量第二维的最大值
            num=0
            sum=0
            # 遍历每个张量，沿着第二维求最大值
            for tensor in teacher_probs:
                max_value= torch.max(tensor, dim=1)[0]  # 只关心最大值，去掉索引
                num+=torch.numel(max_value)
                sum+=torch.sum(max_value)
            avg_fsr=sum/num
            '''

            for img_idx, bbox_result in enumerate(teacher_preds):
                bboxes = []
                for cls_idx, bbox_per_cls in enumerate(bbox_result):   # 将teacher预测的框cat上对应的类别
                ...
                valid_idx = (bboxes[:, -2] > float(avg_fsr)).nonzero()[0] 
                if len(valid_idx) == 0:
                    continue
