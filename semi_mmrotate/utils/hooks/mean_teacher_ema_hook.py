...
class MeanTeacherEMAHook(Hook):
    ...
    def after_train_iter(self, runner):
        ...
        if curr_step % self.interval != 0 or curr_step < self.start_steps:
            return
        if curr_step == self.start_steps:  # 刚开始的一轮momentum为0，初始化权重为student权重
            logger = get_root_logger()
            logger.info(f"Start EMA Update at step {curr_step}")
            self.momentum_update(model, 0)
        else:
            self.momentum_update(model, self.momentum) # 之后每一轮为momentum * teacher + student * (1 - momentum)
