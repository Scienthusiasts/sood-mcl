#!/usr/bin/env python
# -*- coding: utf-8 -*-
# modified from /data/yht/code/sood-mcl/mmrotate-0.3.4/mmrotate/models/dense_heads/rotated_fcos_head.py (add forward_train)
import torch
...
class SparseRotatedBLFCOSHead(RotatedAnchorFreeHead):
    ...
    def forward_train(self,
        ...
        """
        # get_data=True时, 返回的是logits (无监督分支get_data=True)
        if get_data:
            return self(x)
        
        # get_data=False时, 返回的是 loss 字典 (全监督分支get_data=False)
        # NOTE: added by yan, 额外返回flatten_labels(类别GT, 正样本索引), 计算prototype会用到
        loss_dict, flatten_labels, flatten_centerness, flatten_cls_scores, flatten_bbox_preds, flatten_angle_preds = super(SparseRotatedBLFCOSHead, self).forward_train(
            x,
            img_metas,
            gt_bboxes,
            gt_labels=gt_labels,
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, points, regress_ranges,
        ...
        return cls_targets, bbox_targets, angle_targets
